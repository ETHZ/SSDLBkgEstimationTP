...
int DrawInvMassBkgMain(TTree* tree, int leptonId, double Pt_low = 10, double Pt_upp = 50,int nptbins = 10, TString select = "tight", TString effcut = "reliso3", double cut = 0.2);

int DrawInvMassBkg(int leptonId, double Pt_low = 10 , double Pt_upp = 50 ,int nptbins = 10,TString select = "tight", TString effcut = "reliso3", double cut = 0.2){
	...
	//Plot the result
	//
	DrawInvMassBkgMain(tree,leptonId ,Pt_low ,Pt_upp ,nptbins,select,effcut, cut);
	
	return 0;

}

int     DrawInvMassBkgMain(TTree* tree, int leptonId, double Pt_low , double Pt_upp ,int nptbins , TString select, TString effcut , double cut){
	...
	for (int k = 0; k < n; ++k) {
		...
		for (int i = 0; i < Gn; ++i) {
			...
			if((effcut != "dxy")||((effcut == "dxy")&&(Gdxy[i]< cut ))){
			if((effcut != "dz")||((effcut == "dz")&&(Gdz[i]< cut ))){
							...
							for(int ii = 0; ii < nptbins+1; ++ii){
								...
								if((Gpt[l1] > Pt1)&&(Gpt[l1]< Pt2)){
									...
									if((effcut != "reliso4")||((effcut == "reliso4")&&(Giso4[l1] < cut ))){
			                                                ...
			                                                if((effcut != "chiso3")||((effcut == "chiso3")&&(Gchiso3[l1] < cut ))){
			                                                if((effcut != "dxy")||((effcut == "dxy")&&(Gdxy[l1]< cut ))){
			                                                if((effcut != "dz")||((effcut == "dz")&&(Gdz[l1]< cut ))){
