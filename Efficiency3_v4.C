...
* Purpose :  Plot the efficiency for a given cut that is applied to reconstructed data. A matching between the gen. and rec. data is done.
   Same as before but no barrel/endcape separation 
...
int Efficiency(int leptonId, double par_low, double par_upp, int nbins, TString sel_den , TString sel_num, double cut_den = 0., double cut_num = 0., TString par_x = "Pt", TString option = ""){
	...
	else if(sel_num == "tight"){_sel_num = "tightId";}
	...
	//Name of the output
	TString _output = _path+"eff3"+_option+"_"+_pname+"_den_"+_sel_den+"_num_"+_sel_num+"_"+par_x+".root";
	...
	eff->SetLineColor(4);
	eff->SetTitle(_sel_num+" for "+_sel_den+" "+pname+" all #eta");

	//Define the name of the canvas
	TString c1name = "eff"+_option+_pname+"_den_"+_sel_den+"_num_"+_sel_num+"_"+par_x;
