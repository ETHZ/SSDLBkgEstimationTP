...
int TandP(int leptonId, double* par1, int npar1bins, double* par2, int npar2bins, TString sel_den = "tight", TString sel_num = "", double cut_num = 0.2, TString par_x = "Pt", TString par_y = "eta", TString option = "", TString _sig = "CBxBW");

int TandP(int leptonId, double par_low, double par_upp , int npar1bins, TString sel_den = "tight", TString sel_num = "", double cut_num = 0.2, TString par_x = "Pt", TString par_y = "eta", TString option = "", TString _sig = "CBxBW"){
  ...
  if(par_y == "eta"){
    return TandP(leptonId, par1, npar1bins, par2_eta, npar2bins_eta, sel_den, sel_num, cut_num, par_x, par_y, option, _sig);
  }else if(par_y == "pt"){
    return TandP(leptonId, par1, npar1bins, par2_pt, npar2bins_pt, sel_den, sel_num, cut_num, par_x, par_y, option, _sig);
  }else{return 1;}

}

int TandP(int leptonId, double* par1, int npar1bins , double* par2 ,int npar2bins, TString sel_den, TString sel_num, double cut_num, TString par_x, TString par_y, TString option, TString _sig ){
  ...
  TFile  f_test(_path+_fname+".root");
  if(f_test.IsZombie()){

    cout<<"File does not exists"<<endl;
    cout<<"Creating file, please wait..."<<endl;
    DrawInvMassBkg(leptonId,par1, npar1bins,par2, npar2bins, sel_den,sel_num,cut_num,par_x, par_y,option);
    cout<<"Done !"<<endl;

  }

  TFile* file_in 		= new TFile(_path+_fname+".root","read");
  ...
  for(int j = 0; j < npar2bins; ++j){ 
    ...
    for(int i = 0; i<npar1bins; ++i){
      ...
      TString _cname = _path+_fname+"_FIT_PDF/InvM_"+_parxbin+"_"+_parybin;
