/********************************************
* Description - Purpose :  Plot the efficiency for a given cut that is applied to reconstructed data. A matching between the gen. and rec. data is done.
  			   Barrel/endcape are separated 
...
int MC_Ratio(int leptonId, double par_low, double par_upp, int nbins, TString sel_den , TString sel_num, double cut_num = 0., TString par_x = "Pt", TString option = ""){
  ...
  TString _output = _path+ "eff"+_filetag+_option+_pname+"_den_"+_sel_den+"_num_"+_sel_num+"_"+par_x+".root";
  ...
  for (int k = 0; k < n; ++k) {
	...
	if(abs(Gid[j]) == leptonId){
	  ...
	  if((sel_den != "tight")||((sel_den == "tight")&&(Gtight[j] == 1 ))){
    	    //cout<<"the event k is "<<k<<endl;
	    //cout<<"the rec lepton j is "<<j<<endl;
	    //Veto the EE-EB gape
	    ...
	    for (int i = 0; i < ngenPart; ++i) {
	      if((abs(Id[i]) == leptonId)){ 
	    	//cout<<"the gen lepton i is "<<i<<endl;

		//Electrons selection
		//cout<<"eta of the gen is "<<gen_eta[i]<<endl;
		//cout<<"phi of the gen is "<<gen_phi[i]<<endl;
		//cout<<"eta of the rec is "<<Geta[j]<<endl;
		//cout<<"phi of the rec is "<<Gphi[j]<<endl;
  ...
  TString c1name = "eff" +_filetag + option+_pname+"_den_"+_sel_den+"_num_"+_sel_num+"_"+par_x+"_eta<1.2";
  TString c2name = "eff" +_filetag + option+_pname+"_den_"+_sel_den+"_num_"+_sel_num+"_"+par_x+"_eta>1.2";
