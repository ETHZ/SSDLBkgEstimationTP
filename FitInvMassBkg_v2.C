...
* Purpose : Changes w.r.t. v1: takes the histogram of the background and saves an additional plot with the fit of the background and the bkg histogram.
...
int FitInvMassBkg_v2(TH1D* histo,TH1D* histo_bkg, TString _write, TString signal = "BWxGau", TString option = ""){

	//import histo and declare x
	...
	//import histo_bkg and declare x
	int n_bkg = histo_bkg->GetEntries();
	RooPlot* frame_bkg;
	histo_bkg->GetXaxis()->SetRangeUser(hmin0,hmax0);
	// Declare observable x
	RooDataHist dh_bkg("dh","dh",x,Import(*histo_bkg)) ;
	...
	//
	frame_bkg = x.frame(Title(histo_bkg->GetName())) ;
	dh_bkg.plotOn(frame_bkg,DataError(RooAbsData::SumW2), MarkerColor(1),MarkerSize(0.9),MarkerStyle(7));  //this will show histogram data points on canvas
	dh_bkg.statOn(frame_bkg);  //this will display hist stat on canvas
	...
	fit_func->plotOn(frame,Components(bkg),LineStyle(kDashed));
	dh_bkg.plotOn(frame, MarkerColor(2),LineColor(2),MarkerSize(0.9),MarkerStyle(kDot));  //this will show histogram data points on canvas
	

	//plot the bkg on the frame 2
	fit_func->plotOn(frame_bkg,Components(bkg),LineStyle(kDashed));
	...
	frame->Write();
	//frame_bkg->SetName(_write+"_only");
	//frame_bkg->Write();
