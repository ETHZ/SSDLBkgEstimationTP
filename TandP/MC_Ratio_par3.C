...
int MC_Ratio(TString _filetag,int leptonId, double* par1, int npar1bins, double* par2, int npar2bins, TString sel_den , TString sel_num, double cut_num , TString par_x , TString par_y , TString option ){
  ...
  TString _fname = "eff_par3"+_filetag+_option+_pname+_par1range+"_"+_par2range+"_den_"+_sel_den+"_num_"+_sel_num;
  ...
  for (int k = 0; k < n; ++k) {
	...
	evtfromtau[j]              = Gfromtau[j-On];
      ...
      for(int parz_i = 0; parz_i < npar3bins; ++parz_i ){
	if((par_z != "nVert")||((par_z == "nVert")&&((evt_nvert >= par3[parz_i])&&(evt_nvert < par3[parz_i + 1])))){
